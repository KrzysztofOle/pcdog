...
class HardwareAgentTests(unittest.TestCase):
    ...
    def test_open_collector_input_configuration_is_active_low_with_pull_up(self) -> None:
        ...
        hardware_unit = (root / "systemd" / "pcdog-hardware-agent.service").read_text()
        init_unit = (root / "systemd" / "pcdog-gpio-input-init.service").read_text()
        installer = (root / "scripts" / "install-runtime.sh").read_text()
        self.assertIn("Requires=pcdog-gpio-input-init.service", hardware_unit)
        self.assertIn("After=local-fs.target pcdog-gpio-input-init.service", hardware_unit)
        self.assertNotIn("ExecStartPre=", hardware_unit)
        self.assertIn("ProtectKernelTunables=yes", hardware_unit)
        self.assertIn("Type=oneshot", init_unit)
        self.assertIn("RemainAfterExit=yes", init_unit)
        self.assertIn("Before=pcdog-hardware-agent.service", init_unit)
        self.assertIn("ExecStart=/usr/bin/pinctrl set 19 ip pu", init_unit)
        self.assertIn("ExecStart=/usr/bin/pinctrl set 20 ip pu", init_unit)
        self.assertIn("DevicePolicy=closed", init_unit)
        self.assertIn("DeviceAllow=/dev/gpiomem rw", init_unit)
        self.assertNotIn("ProtectKernelTunables=yes", init_unit)
        self.assertIn("GPIO_INPUT_INIT_SERVICE_NAME='pcdog-gpio-input-init.service'", installer)
        self.assertIn('systemctl enable "$GPIO_INPUT_INIT_SERVICE_NAME"', installer)
        self.assertIn('systemctl is-active --quiet "$GPIO_INPUT_INIT_SERVICE_NAME"', installer)
        completed = Mock(stdout="20=active 19=inactive\n")
